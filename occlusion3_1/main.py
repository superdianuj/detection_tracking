...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, offset=(0,0)):
    for i, box in enumerate(bbox):
        ...
        x1 += offset[0]
        x2 += offset[0]
        y1 += offset[1]  # Corrected offset for y
        y2 += offset[1]  # Corrected offset for y
...
output = cv2.VideoWriter('ref_vid_detected.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, (frame_width, frame_height))
    ...
    if ret:
        ...
        if len(xywh_bboxs) > 0:
            ...
            if len(outputs) > 0:
                ...
                object_id = outputs[:, -1]
                draw_boxes(frame, bbox_xyxy, identities, object_id, names)
